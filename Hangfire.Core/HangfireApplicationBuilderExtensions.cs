...
namespace Hangfire
{
    ...
    {
        public static IApplicationBuilder UseHangfirePlugin(
        [NotNull] this IApplicationBuilder app,
        [NotNull] string pluginPath)
        {
            ...
            var path = PathExtensions.MapPath(pluginPath, env.ContentRootPath);
            var paths = Directory.GetDirectories(path).ToList();
            var nodeServices = services.GetRequiredService<INodeServices>();
            NodePluginBridge.Init(nodeServices);
            PluginService.Register(paths);
